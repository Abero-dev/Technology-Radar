...
export const ServiceCard: React.FC<ServiceCardProps> = ({ icon, title, options, onOptionClick }) => {
    return (
            ...
            <ul className="flex flex-col justify-center items-center gap-[20px] p-0">
                {options.map((option, index) => (
                    <li key={index} className="list-none transition-transform duration-[200ms] ease-in-out hover:scale-110" onClick={onOptionClick}>
                        <NavLink
                            to={option.route}
                            className="no-underline text-[#eee] text-[26px] text-right cursor-pointer rounded-[12px] px-[8px] py-0 hover:border-b-4 hover:border-[#eee]"
                        >
                            {option.name}
                        </NavLink>
                    </li>
